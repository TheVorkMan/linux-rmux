...
/*
 * xhci-ps4.c - xHCI host controller driver for Sony PS4 (Aeolia/Belize/Baikal)
 *
...
#include <asm/ps4.h>
#include "xhci-ps4.h"
#include "../../ps4/ps4-aeolia.h"

#include "xhci.h"

static const char hcd_name[] = "xhci_ps4";

static struct hc_driver __read_mostly xhci_ps4_hc_driver;
...
static int xhci_ps4_setup(struct usb_hcd *hcd);

static const struct xhci_driver_overrides xhci_ps4_overrides __initconst = {
	.extra_priv_size = sizeof(struct xhci_hcd),
	.reset = xhci_ps4_setup,
};

static void xhci_ps4_quirks(struct device *dev, struct xhci_hcd *xhci)
{
	/*
...
static int xhci_ps4_setup(struct usb_hcd *hcd)
{
	return xhci_gen_setup(hcd, xhci_ps4_quirks);
}

static int xhci_ps4_probe_one(struct pci_dev *dev, int index)
{
	int retval;
	struct ps4_xhci *axhci = pci_get_drvdata(dev);
	struct hc_driver *driver = &xhci_ps4_hc_driver;
	struct usb_hcd *hcd;
	struct xhci_hcd *xhci;
	int irq = (axhci->nr_irqs > 1) ? (dev->irq + index) : dev->irq;

	dev_dbg(&dev->dev, "xhci_ps4_probe_one %d\n", index);
...
static void xhci_ps4_remove_one(struct pci_dev *dev, int index)
{
	struct ps4_xhci *axhci = pci_get_drvdata(dev);
...
static int ahci_init_one(struct pci_dev *pdev)
{
	...
	struct ps4_xhci *axhci = pci_get_drvdata(pdev);
...
static int xhci_ps4_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
	int idx;
	int retval;
	bool ahci_started = false;
	struct ps4_xhci *axhci;
	...
	for (idx = 0; idx < NR_DEVICES; idx++) {
		...
		retval = xhci_ps4_probe_one(dev, idx);
		if (retval)
			goto remove_hcds;
	}
...
static void xhci_ps4_remove(struct pci_dev *dev)
{
	int idx;
	struct ps4_xhci *axhci = pci_get_drvdata(dev);
	if (!axhci)
...
static int xhci_ps4_suspend(struct device *dev)
{
	int idx;
	struct ps4_xhci *axhci = dev_get_drvdata(dev);
	struct xhci_hcd	*xhci;
	int retval;
...
static int xhci_ps4_resume(struct device *dev)
{
	int idx;
	struct ps4_xhci *axhci = dev_get_drvdata(dev);
	struct xhci_hcd	*xhci;
...
static const struct dev_pm_ops xhci_ps4_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(xhci_ps4_suspend, xhci_ps4_resume)
};
#endif

/* pci driver glue; this is a "new style" PCI driver module */
static struct pci_driver xhci_ps4_driver = {
	.name =		"xhci_ps4",
	.id_table =	pci_ids,

	.probe =	xhci_ps4_probe,
	.remove =	xhci_ps4_remove,
...
static int __init xhci_ps4_init(void)
{
	xhci_init_driver(&xhci_ps4_hc_driver, &xhci_ps4_overrides);
	return pci_register_driver(&xhci_ps4_driver);
}
module_init(xhci_ps4_init);

static void __exit xhci_ps4_exit(void)
{
	pci_unregister_driver(&xhci_ps4_driver);
}
module_exit(xhci_ps4_exit);

MODULE_AUTHOR("fail0verflow; crashniels; rmuxnet");
MODULE_DESCRIPTION("xHCI Sony PS4 Host Controller Driver");
MODULE_LICENSE("GPL");
